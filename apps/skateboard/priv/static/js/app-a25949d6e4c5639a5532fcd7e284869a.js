...
Elm.GameBoard.make = function (_elm) {
   ...
   var _op = {};
   var update = F2(function (action,model) {    var _p0 = action;if (_p0.ctor === "Submit") {    return model;} else {    return model;}});
   var Submit = {ctor: "Submit"};
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([]),_U.list([$Html.text($Basics.toString(model))]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Submit)]),_U.list([$Html.text("Click")]))]));
   });
   var NoOp = {ctor: "NoOp"};
   var model = "cat";
   var main = $StartApp$Simple.start({model: model,view: view,update: update});
   return _elm.GameBoard.values = {_op: _op,model: model,NoOp: NoOp,Submit: Submit,update: update,view: view,main: main};
};
...
var elmApp = Elm.embed(Elm.GameBoard, elmDiv);
});
